...
export default async function PipelinePage() {
  ...
  try {
    ...
    if (!reqRes2.error) requisitions = reqRes2.data ?? [];
    ...
    await sweepStepTatBreaches(supabase, candidates, requisitionById, org, templates);
    ...
    await processDuePendingNotifications(supabase, org);

    const candRes3 = await supabase.from("candidates").select("*").eq("archived", false).order("created_at", { ascending: false });
    if (!candRes3.error) candidates = candRes3.data ?? [];
